...
RaydiumLPV4_str = "675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8"
RaydiumLPV4 = Pubkey.from_string(RaydiumLPV4_str)
...
async def get_tokens(signature: Signature, RaydiumLPV4: Pubkey) -> None:
    ...
    for instruction in filtred_instuctions:
        Token0, Token1, Pair = get_tokens_info(instruction)  # Modified this line
        # print_table((Token0, Token1, Pair))  # Adjusted to tuple
        #print(f"True, https://solscan.io/tx/{signature}")
        
        detected_amm_id = str(Pair)  
        await get_data(detected_amm_id)
...
def get_tokens_info(
    ...
    logging.info(f"\n Token0: {Token0}, \n Token1: {Token1}, \n Pair: {Pair}")
    # End logging
    # Write to lp_accounts.txt
    # with open("lp_accounts.txt", 'a', encoding='utf-8') as lp_file:
    #     lp_file.write(f"Token0: {Token0}, Token1: {Token1}, amm_id: {Pair}\n")
...
#         {'Token_Index': 'amm_id', 'Account Public Key': tokens[2]}  # amm_id
#     ]
